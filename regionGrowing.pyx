...
class regionGrow():
    def __init__(self, im_path, th):
        self.video = cv2.VideoCapture(im_path)
        self.thresh = float(th)
        self.success, self.im = self.video.read()
        turns = 0
        ...
        while self.success == True:
            ...
            self.stack = Stack()
            self.spatialGraph = None
            self.ApplyRegionGrow()
            self.success, self.im = self.video.read()
    ...
    def ApplyRegionGrow(self):
        t = time.time()
        for x0 in range(0, self.h):
            for y0 in range(0, self.w):
                self.grown(x0, y0)
        ...
        print(t2 - t)
        print(self.passedBy)
        self.spatialGraph = spatialGraph.createSpatialGraph(self.w,self.h,self.passedBy,self.currentRegion)
